...
const TasksViewColumn = (props) => {
  ...
  useEffect(() => {
    const project = JSON.parse(localStorage.getItem("lastProjectSelected"))
  ...
  return (
    ...
    <>
      {addingNewBoard && (
        ...
        >
          ...
          <div className="flex items-center ">
            ...
            <button className="text-xs text-white p-1 rounded mx-1 bg-red-500" onClick={()=>{setAddingNewBoard(false); setNewBoardValue("")}} >بستن</button>
            <button className="text-xs text-white p-1 rounded mr-1 bg-[#208D8E]" onClick={addNewBoard} >افزودن</button>
            <input className="border border-gray-200 rounded pr-1 mr-1 text-sm" onChange={(e) => setNewBoardValue(e.target.value)} value={newBoardValue} />
          </div>
        </div>
      )}
      <div
      ...
      >
        <div className=" flex items-center justify-center h-[25px] gap-2 opacity-0 group-hover/container:opacity-100 transition-opacity duration-200 ease-in"></div>
        <div onClick={() => setAddingNewBoard(true)} className="flex items-center ">
          {/* <span className="inline-flex bg-slate-100 w-[20px] justify-center items-center rounded-lg">
            {props.count}
          </span> */}
          <span
